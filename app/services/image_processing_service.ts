...
export default class ImageProcessingService {
  ...
  async processAndUpload(
    ...
    const [fullImage, thumbImage] = await Promise.all([
      this.processImage(pipeline.clone(), this.FULL_SIZE),
      this.generateThumbnail(pipeline.clone()),
    ])
  ...
  private async processImage(
    ...
    try {
      const { data: buffer, info } = await instance
        .resize(maxSize, maxSize, {
          fit: 'inside',
          withoutEnlargement: true,
        })
  ...
  private async generateThumbnail(instance: sharp.Sharp): Promise<ProcessedImage> {
    try {
      const { data: buffer, info } = await instance
        .resize(this.THUMB_SIZE, this.THUMB_SIZE, {
          fit: 'cover',
